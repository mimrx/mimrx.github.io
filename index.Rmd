...
The plots compared to each other show clearly that the speechiness always follows the same structure. In each case there is exactly one outlier which is way more speechy than the other songs. Also the speechiness of these outliers and in general is almost the same in each composition except from *Goodnight Mommy*. But this exceptation makes sense because the first three compositions are for musical performances and *Goodnight Mommy* is a soundtrack album for a film.

The fact that the range 0.33 and 0.66 describes tracks that may contain both music and speech shows what I meant earlier (see "Tempo & Chaos - happy, sad & something inbetween"). She surely uses instruments but not in a "stereotypical" way. She 
...
### Different songs, same sound?

The respond of the critique from my friend that "every song sounds the same" is that they definitely not. There are some structures she followed through her compositions.
