...
bählammsfest %>% mutate(mode = ifelse(mode == 0, 'Minor', 'Major')) %>% ggplot(aes(x = speechiness, y = loudness, colour = mode)) + geom_point(alpha = 0.6) + theme_minimal() + ggtitle("Bählamms Fest")

dertodunddasmädchenII <- get_playlist_audio_features('applejackey', '4XW6Ch7smd3Ngwh7RNQmck')

dertodunddasmädchenII %>% mutate(mode = ifelse(mode == 0, 'Minor', 'Major')) %>% ggplot(aes(x = speechiness, y = loudness, colour = mode)) + geom_point(alpha = 0.6) + theme_minimal() + ggtitle("The death and the girl II")

losthighway <- get_playlist_audio_features('applejackey', '0rpE8eKsaal3ovP4TAYNgZ')

losthighway %>% mutate(mode = ifelse(mode == 0, 'Minor', 'Major')) %>% ggplot(aes(x = speechiness, y = loudness, colour = mode)) + geom_point(alpha = 0.6) + theme_minimal() + ggtitle ("Lost Highway")

goodnightmommy <- get_playlist_audio_features('applejackey', '6cZmLiip5N2zcJCbL7blFu')

goodnightmommy %>% mutate(mode = ifelse(mode == 0, 'Minor', 'Major')) %>% ggplot(aes(x = speechiness, y = loudness, colour = mode)) + geom_point(alpha = 0.6) + theme_minimal() + ggtitle("Goodnight Mommy")
```
